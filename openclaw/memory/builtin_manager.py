...
class BuiltinMemoryManager:
    ...
    async def _fts_search(
        self, query: str, limit: int, sources: list[MemorySource] | None
    ) -> list[MemorySearchResult]:
        ...
        try:
            ...
            cursor = self.db.execute(sql, [query] + source_values + [limit])
    ...
    async def add_file(self, file_path: Path, source: MemorySource = MemorySource.MEMORY) -> int:
        """
        ...
        try:
            ...
            self.db.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """,
                [str(file_path), source.value, file_hash, int(stat.st_mtime), stat.st_size, now],
            )
    ...
    async def sync(self) -> dict:
        ...
        Returns:
            Sync statistics
        """
        stats = {"files_added": 0, "files_updated": 0, "files_removed": 0, "chunks_created": 0}
