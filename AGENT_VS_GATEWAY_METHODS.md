...
## 延伸问题：为什么需要 WebSocket？

**问题**：调用这些系统管理方法（config.get、channels.status 等），为什么不能用 HTTP，必须用 WebSocket？

**简短答案**：
- 不是因为"系统管理功能"需要 WebSocket
- 而是因为 **Agent 的流式输出** 必须用 WebSocket
- Gateway 为了统一接口，把所有方法都放在 WebSocket 上

**详细说明**：参见 [WHY_WEBSOCKET.md](WHY_WEBSOCKET.md)

### 核心原因

| 需求 | HTTP | WebSocket |
|------|------|-----------|
| Agent 流式输出（逐字显示） | ❌ 必须等完整响应 | ✅ 实时推送 |
| 实时事件广播（监听其他 channel） | ❌ 需要轮询 | ✅ 主动推送 |
| 双向通信（中止任务） | ❌ 请求后无法取消 | ✅ 随时发送 |
| 长时间运行（30秒+） | ⚠️ 容易超时 | ✅ 持久连接 |

### 示例对比

**HTTP 方案（不实用）**：
```bash
$ curl -X POST /api/agent -d '{"message": "分析代码"}'
# ⏳ 等待 45 秒... 看不到任何进度
# 45 秒后一次性返回完整响应
{"text": "这个项目采用了...（3000字）"}
```
...
> {"method": "agent", "params": {"message": "分析代码"}}
< {"type": "thinking", "text": "正在读取文件..."}  # T=1s
